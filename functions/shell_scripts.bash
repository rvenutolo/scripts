...
# Return true if the given file's first line contains a bash or sh shebang.
# $1 = file path
...
# Emit shell-script paths under SCRIPTS_DIR when called with no args; excludes
# /other/ (third-party copies) and /test/bats/, /test/test_helper/bats-support/,
# /test/test_helper/bats-assert/ (vendored BATS submodules). With args: each
# arg is either a directory (recursed via `shfmt --find`) or a file (emitted
# as-is). Caller must validate that each arg exists before calling.

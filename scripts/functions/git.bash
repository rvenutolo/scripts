...
# @description Unset every repo-scoped environment variable that git injects into hook and child-process environments
#              (GIT_DIR, GIT_WORK_TREE, GIT_INDEX_FILE, ...). A worktree `git push` exports an absolute GIT_DIR into
#              hooks — the fixture-escape hazard — so any tooling that then runs git against OTHER repos gets silently
#              retargeted at the pushing repo. The list comes from `git rev-parse --local-env-vars` at runtime so new

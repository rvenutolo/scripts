...
- `scripts/interactive/` — everything else: utility scripts that prompt, launch a GUI, drive a picker, or otherwise assume a terminal, **plus every wrapper** (the former `wrapper/` scripts — `mvn`, `gradle`, `kate`, the flatpak wrappers, etc.). On `PATH` only in interactive shells (the user wires this into `~/.bashrc` behind a `case $- in *i*)` guard). **Exception:** the `claude` wrapper lives in `scripts/non-interactive/` — it derives `CLAUDE_CONFIG_DIR` from `PWD` at launch (honoring an already-set value), and that selection must also cover cron/`ssh`/scripted launches where `scripts/interactive/` is not on `PATH`. It also places an interactive session launched from a plain terminal into a herdr workspace (starting the server if needed, creating a workspace for `PWD`, typing `claude <args>` into its root pane, then attaching), and passes straight through when `HERDR_ENV` is already set — which is what the `claude` typed into that pane hits. `CLAUDE_NO_HERDR=1`, a non-TTY stdin/stdout, a missing `herdr`, or any non-session argument (`-p`, `--version`, a subcommand such as `mcp`) also bypass it. herdr joins the typed words with spaces and the pane shell re-parses them, so each argument is `${arg@Q}`-quoted first; and panes inherit the server's environment, so the server is started with `CLAUDE_CONFIG_DIR` unset and each workspace gets its profile via `--env`.
...
`*.yaml`, `*.toml` and the extensionless top-level executables, plus every dotfile regardless of a
further extension (`.envrc`, `.editorconfig`, `.justfile` and the like) — minus `scripts/other/`,
`.shdoc/` and `site/`. The `.shdoc` submodule gitlink is skipped too: it is not a readable regular
file. Detection is whole-line rather than comment-aware — deciding whether a `#` opens a comment
needs a shell parser, and runtime strings are in scope anyway — so `${#output}`, `$#` and `#L12`
line anchors are excluded by the pattern rather than by context. A six-hex colour is not
automatically excluded: prefixed with the hash sign, a value such as `0000ff` is exactly four long
(a three-long run, e.g. `123abc`, matches the same way), while `123456` does not because that run
continues past four; no colour in the tree today falls on the matching side. **Markdown is
deliberately out of scope**: this file carries issue numbers as provenance for decisions a reader
is told not to relitigate, and it must keep them.
